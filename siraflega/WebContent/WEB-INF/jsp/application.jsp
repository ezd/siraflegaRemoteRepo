	...
	<div class="col-md-8">
		<div class="container-fluid container-holder">
			<div class="row outPutRow ">
				<div class="col-xs-12 mainTiltle" style="margin: 0px; padding: 0px;">
					<p style="float: right">
						...
						English <input type="radio" name="languageOption" value="amharic">
						አማርኛ
					</p>
			...
			<input type="hidden" id="jobId" value="${postedJob.id}"> <input
				type="hidden" id="empId" value="${employee.id}"> <br>
			<div class="row outPutRow">
				<div class="col-xs-12"
					style="margin: 0px; padding: 0px; text-align: justify;">
					<c:choose>
						<c:when
							test="${empty employee or employee.summary ==''or empty employee.summary }">
							<p id="yoursummary">
								<a
									href="${pageContext.request.contextPath}/account.html?returnTo=apply&&jobid=${postedJob.id}">Your
									summary is not Complete. Please build your profile and apply
									again.</a>
							</p>
						</c:when>
					</c:choose>
					<p id="jobSummary">${employee.summary}<a
							href="${pageContext.request.contextPath}/account.html?returnTo=apply&&jobid=${postedJob.id}"><span
							class="glyphicon glyphicon-pencil" id="btnSign"></span></a>
					</p>
				</div>
			</div>
			<div class="row outPutRow">
				<div class="col-xs-12"
					...
					<p class="values amharicContent">
						ይህን ደብዳቤ ለመጻፍ የተነሳሳሁት <strong>Siraflega&copy;</strong> ድህረ ገጽ ላይ
						ባገኘሁት መረጃ መሰረት ነው：： ባገኘሁት መረጃ መሰረት, <strong>${postedJob.company.name}</strong>
						ባለው ክፍት ቦታ <strong>${postedJob.position} </strong> አወዳድሮ ለመቅጠር
						እንደሚፈልግ ለመረዳት ችያለሁ：：
					...
					<p class="amharicContent">
						...
						<c:if test="${isApplied eq false}">
							<button class="btn btn-link" data-toggle="modal"
								data-target="#letterModal" id="addLetter" style="float: left;">
								<span class="glyphicon glyphicon-pencil btnSign"
									style="float: right;"></span> <span class="btnText">ለምን
									ለክፍት ስራ ቦታው ጥሩ ተወዳዳሪ እንደሆንክ ግለጽ </span>
							</button>
						</c:if>
					</p>
				</div>
				...
				<div class="col-xs-12"
					...
					<c:choose>
						<c:when test="${isApplied eq true}">
							<p class="values engContent">
								Thank you. You have applied for this position.<span
							...
							<p class="values amharicContent">
								እናመሰግናለን። ማመልከቻውን በሚገባ አጠናከዋል።<span class="input-group-btn"><button
										class="btn btn-info" id="btnBack">
										<span class="glyphicon glyphicon-chevron-left"></span>ተመለስ
									</button></span>
							</p>
						</c:when>
		...
		<div class="modal-dialog overflowstyle" role="document"
			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title engContent" id="myModalLabel">Why you
						are a big fit for the position?</h4>
					<h4 class="modal-title amharicContent" id="myModalLabel">ለምን
						ለስራው ተገቢ ምርጫ እንደሆንክ ግለጽ።</h4>
				</div>
				<div class="modal-body">
					<textarea id="letterTxt" maxlength="1200"
...
<script type="text/javascript">
	...
	function changeLanguge(langoption) {
		...
		$("input[name='languageOption'][value=" + langoption + "]").prop(
				'checked', true);
	...
	$(document)
			.ready(
					function() {
						...
						langoption = $('#languageflag').val() == '' ? langoption
								: $('#languageflag').val();
						...
						$('#saveChangeBtn').on(
								...
								function() {
									...
									if (nicEditors.findEditor("letterTxt")
											.getContent() == '') {
										$(".btnSign").attr('class',
												...
												$
														.ajax({
															...
															success : function(
																	data) {
																window.location
																		.replace('${pageContext.request.contextPath}/apply/'
																				+ $(
																						'#jobId')
																						.val()
																				+ "?langoption="
